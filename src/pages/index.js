...
export default function Home() {
  ...
  useEffect(() => {
    ...
    wallet.viewMethod({ contractId: CONTRACT, method: 'get_epoch_by_id', args: { epoch_id : "1"} }).then(
      epoch => {
        console.log(epoch)
        setEpoch(epoch)}
    );
  ...
  const verifyProof = async (proof) => {
    setShowSpinner(true);
    const newProof = Reclaim.transformForOnchain(proof)
    ...
    const tx = await wallet.callMethod({ contractId: CONTRACT, method: 'verify_proof', args: { proof: newProof } });
    console.log(tx)
    setTransaction(tx);
    ...
    <main className={styles.main}>
      <div className={styles.description}>
        <p>
          ...
          <code className={styles.code}>Reclaim Contract on Near - Testnet</code>
        </p>
      </div>

      <div>
        <h1 className="w-100">
          The contract says - Current Epoch
        </h1>
        {
          typeof epoch == 'string' ? <div>{epoch}</div> :
            <div>
              <code>
                id : {epoch.id} <br />
              </code>
              <code>
                Minimum Witnesses for Claim Creation: {epoch.minimum_witness_for_claim_creation} <br />
              </code>
              <code>
                Witnesses: { epoch.witnesses.map(w => w.address)}
              </code>
            </div>
        }
      </div>
      ...
      <div hidden={!loggedIn}>
          <div className={styles.col}>
            <button className="btn btn-secondary" onClick={() => verifyProof(SMOKE_PROOF)}>
            ...
            <div hidden={readyToVerify}>
              <CreateNewProof setNewProof={setNewGenProof} setReadyToVerify={setReadyToVerify} />
            </div>
            <div hidden={!readyToVerify} className={styles.description}>
              <button className="btn btn-secondary" onClick={() => verifyProof(newGenProof)}>
